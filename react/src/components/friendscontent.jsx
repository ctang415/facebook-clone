...
const FriendsContent = () => {
    ...
    if (allFriends && !friendsRequest) {
        return (
                                ...
                                {x.users.filter( y => y.id !== userData.id).map(user => {
                                    return (
                                        <Link to={`/profiles/${user.id}`} key={user.id}>
                                            <li className="flex flex-row border-1 bg-white p-6 rounded-md shadow-md justify-between min-w-[20vw] min-h-[10vh] items-center" key={user.id}>
                ...
                <div className={ userData.friends && userData.friends.filter(friend => friend.status === "Friends").length === 0 ? "flex flex-col min-h-full min-w-full justify-center items-center" : "hidden"}>
            ...
            <div className="min-h-full w-screen pt-20">
                <div className="items-center justify-center">
                    ...
                    <ul className="flex flex-row gap-6 justify-center flex-wrap">
                        { userData.friends && userData.friends.filter(friend => friend.status === "Pending").map(x => {
                            return (
                                <div key={x.id}> 
                                    {x.users.filter( y => y.id !== userData.id).map(user => {
                                        return (
