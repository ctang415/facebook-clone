...
exports.friend_detail_get = asyncHandler( async (req, res, next) => {
    const searchFriend = await User.find({first_name : req.body.first_name}).select('first_name last_name avatar')
...
exports.friend_update = asyncHandler( async (req, res, next) => {
    const friend = await Friend.findByIdAndUpdate(req.body.id, {status: "Friends"})
    res.status(200).json({friend: friend, success: true})
})

exports.friend_delete = asyncHandler ( async (req, res, next) => {
    const friend = await Friend.findById(req.body.id)
    if (friend === null) {
        res.status(404).json({error: "Friend not found."})
        return
    }
    const friendRequest = await Friend.findByIdAndRemove(req.body.id)
    const user = await User.findByIdAndUpdate(req.body.userid, { $pull: {friends: req.body.id }})
    const userFriend = await User.findByIdAndUpdate(req.body.friendid, {$pull: {friends: req.body.id} })
